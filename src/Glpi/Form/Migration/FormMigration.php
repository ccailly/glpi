...
final class FormMigration extends AbstractPluginMigration
{
    ...
    {
        $this->addMessage(MessageType::Notice, 'Checking plugin version...');

        $plugin = new Plugin();
        if (!$plugin->getFromDBbyDir('formcreator')) {
            $this->addMessage(MessageType::Error, 'Formcreator plugin is not installed.');
            return false;
        }

        $is_version_ok = FormCreatorPluginToCoreCommand::FORMCREATOR_REQUIRED_VERSION === $plugin->fields['version'];
        if (!$is_version_ok) {
            $this->addMessage(MessageType::Error, sprintf(
        ...
        foreach ($formcreator_tables as $table) {
            if (!$this->db->tableExists($table)) {
                $this->addMessage(MessageType::Error, sprintf('Formcreator plugin table "%s" is missing.', $table));
                $missing_tables = true;
            }
        }
        if ($missing_tables) {
            $this->addMessage(MessageType::Error, 'Migration cannot be done.');
            return false;
        }
