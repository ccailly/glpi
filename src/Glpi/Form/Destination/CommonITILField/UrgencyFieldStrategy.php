...
enum UrgencyFieldStrategy: string
{
    ...
    public function getUrgency(
        ...
        return match ($this) {
            ...
            self::SPECIFIC_ANSWER => $this->getUrgencyForSpecificAnswer(
                $config->getSpecificQuestionId(),
                $answers_set
            ),
            self::LAST_VALID_ANSWER => $this->getUrgencyForLastValidAnswer($answers_set),
        };
    }

    private function getUrgencyForSpecificAnswer(
        ?int $question_id,
        AnswersSet $answers_set,
    ): ?int {
        if ($question_id === null) {
    ...
    private function getUrgencyForLastValidAnswer(
        AnswersSet $answers_set,
    ): ?int {
